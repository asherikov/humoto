...
function(humoto_define_regression_test HUMOTO_MODULE REGRESSION_TEST_ID DEPENDENCIES)
    ...
    if(DIFF_WITH_REFERENCE)
        ...
        if(HUMOTO_BUILD_WITH_QI)
            qi_add_test(${TGT_NAME} "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${TEST_NAME}" ARGUMENTS "${HUMOTO_MODULE_DIR}/config/" "${TEST_NAME}.ref" WORKING_DIRECTORY "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}")
        else()
            add_test(NAME ${TGT_NAME} COMMAND ${TEST_NAME} "${HUMOTO_MODULE_DIR}/config/" "${TEST_NAME}.ref" WORKING_DIRECTORY ${CMAKE_RUNTIME_OUTPUT_DIRECTORY})
        endif()
    endif()
endfunction()
